...
export default function WelcomePage() {
  ...
  // Post-checkout pending activation
  ...
  // If we arrive from Stripe: fetch email via /api/claim and send OTP
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Fire the OTP magic link; redirect after clicking will come back to /welcome
        ...
        setPendingInfo(
          "Check your email to activate your account. Once confirmed, we’ll finish setup here."
        );
      } catch (e: any) {
        setPendingInfo(
    ...
    <Container>
      ...
      {!user && pendingEmail && (
        <div className="mb-4 rounded-lg border border-amber-400/30 bg-amber-400/10 p-3 text-sm text-amber-100">
          <div className="font-medium">Activate your account</div>
          <div className="mt-1">
            We’ve sent a sign-in link to <span className="font-mono">{pendingEmail}</span>. Click it
            to confirm and return here to finish setup.
          </div>
        </div>
      )}
      {!user && !pendingEmail && (params.get("pending") === "1") && (
        <div className="mb-4 rounded-lg border border-amber-400/30 bg-amber-400/10 p-3 text-sm text-amber-100">
          {pendingInfo || "Finalising your membership…"}
        </div>
      )}

      {/* The rest of your existing page remains unchanged below */}
      {/* Card + Next steps */}
