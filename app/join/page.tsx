...
const API_BASE = (process.env.NEXT_PUBLIC_API_BASE ?? "https://tradescard-api.vercel.app").replace(/\/$/, "");
...
export default function JoinPage() {
  ...
  useEffect(() => {
    ...
    if (qFree === "1") { setFreeOpen(true); setOpenInline(null); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [params]);
  ...
  // ---------- if already signed in and a plan is opened inline, jump to Stripe ----------
  useEffect(() => {
    (async () => {
  ...
  async function sendMagicLink(targetEmail: string, nextPath = "/offers") {
    const trimmed = targetEmail.trim();
    if (!trimmed || !isValidEmail(trimmed)) throw new Error("invalid_email");
    const { error: supaErr } = await supabase.auth.signInWithOtp({
      email: trimmed,
      options: { emailRedirectTo: new URL(nextPath, APP_URL).toString() },
    });
    if (supaErr) throw supaErr;
  }

  // The ONLY place that calls the checkout API
  async function startPaidCheckout(plan: Exclude[Plan, "access"], emailForCheckout: string, userId?: string) {
    try {
      setBusy(true);
      setCheckoutError("");

      // Hard gate: we must have a session/user id for paid checkout
      if (!userId) {
        // If we got here without a session (shouldn’t happen now), fall back to magic link
  ...
  async function handleFreeJoin() {
    ...
    try {
      ...
      setSent(false);
      setInfo((e as Error)?.message === "invalid_email" ? "Enter a valid email to get your sign-in link." : "We couldn’t send the link just now. Please try again.");
  ...
  // User clicked a paid plan tile
  async function choose(plan: Exclude<Plan, "access">) {
    setInfo("");
    const { data } = await supabase.auth.getSession();

    if (!data?.session?.user) {
      // Logged out: open inline email capture
      setOpenInline(plan);
      setFreeOpen(false);
  ...
  // Submit paid email inline (logged-out)
  async function handlePaidContinue() {
    ...
    try {
      ...
      // For paid routes we require a session. Send magic link that bounces back to this plan.
      const next = `/join?plan=${openInline}`;
      await sendMagicLink(emailPaid, next);
      setSent(true);
      setInfo("Check your inbox for a secure sign-in link. Once you’re in, we’ll take you to payment.");
    } catch (e: any) {
      setCheckoutError(e?.message || "We couldn’t start checkout. Please try again.");
        ...
        <div className="inline-flex rounded-xl border border-neutral-800 p-1">
          {(["month", "year"] as const).map((c) => (
            <button
              key={c}
              onClick={() => setCycle(c)}
              className={`px-3 py-1.5 text-sm rounded-lg ${cycle === c ? "bg-neutral-800" : "hover:bg-neutral-900"}`}
    ...
    return (
      <div className="mb-4 grid gap-2 sm:grid-cols-3">
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-3">
          ...
          <p className="mt-1 text-sm text-neutral-300">Real trade deals that cut everyday costs — tools, fuel, food & essentials.</p>
        </div>
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-3">
          <div className="text-sm font-semibold">Benefits</div>
          <p className="mt-1 text-sm text-neutral-300">Useful inclusions from day one, plus member-only partner pricing.</p>
        </div>
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-3">
          <div className="text-sm font-semibold">Rewards</div>
          <p className="mt-1 text-sm text-neutral-300">Earn points each month — entries to regular prize draws. Cancel any time.</p>
        </div>
      </div>
    );
  }
  ...
  function PlanCard({
    plan, title, features, accent, badge,
  }: {
    ...
    const ctaLabel =
      plan === "member" && cycle === "month" && showTrial ? TRIAL_COPY
      : plan === "member" ? `Choose Member – ${price}`
      : `Choose Pro – ${price}`;
    ...
    return (
      <div className={`rounded-2xl border ${accentCls} bg-neutral-900 p-4`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => !selected && setSelectedPlan(plan)}
          ...
          <div className="flex items-center gap-2">
            {badge && <span className="rounded-full border border-amber-400/30 bg-amber-400/10 px-2 py-0.5 text-xs text-amber-300">{badge}</span>}
        ...
        {!isInline ? (
          <PrimaryButton onClick={(e) => { e.stopPropagation(); choose(plan); }} disabled={busy || sending} className="mt-4 w-full">
            {ctaLabel}
          </PrimaryButton>
        ) : (
          <div className="mt-4 grid gap-2 sm:grid-cols-[1fr_auto]">
            ...
            <p className="col-span-full text-xs text-neutral-500">
              We’ll email you a secure link first. After sign-in, we’ll take you to payment.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 md:hidden border-t border-neutral-800 bg-neutral-950/95 backdrop-blur supports-[backdrop-filter]:bg-neutral-950/70" role="region" aria-label="Selected plan summary">
          ...
          <div className="mx-auto max-w-5xl px-4 py-3">
            ...
            <div className="mt-2 text-center text-[11px] text-neutral-400">
              {cycle === "year" ? `${fmtMonth(PRICE[selectedPlan].month)} equivalent • ${monthsFree(PRICE[selectedPlan].month, PRICE[selectedPlan].year).months > 0 ? `${monthsFree(PRICE[selectedPlan].month, PRICE[selectedPlan].year).months} months free` : "Save vs monthly"}` : `Billed monthly • Cancel any time`}
            </div>
          </div>
        </div>
      )}
          ...
          <>
            <div className="grid gap-4 md:grid-cols-2">
              <PlanCard
                plan="member"
                title="Member"
                features={["All offers unlocked", "Member benefits included", "Monthly rewards entries", "Digital card"]}
                badge="Most popular"
                accent="member"
              />
              <PlanCard
                plan="pro"
                title="Pro"
                features={["Everything in Member", "Early access & Pro-only offers", "Bigger rewards boosts"]}
                accent="pro"
              />
            </div>
